...
import { deviceScopeFromRequest, projectGuidelinesDir } from "@/lib/server/db";

export async function GET(req: NextRequest) {
  const scope = deviceScopeFromRequest(req);
  const dir = projectGuidelinesDir(undefined, scope);
  ...
  return NextResponse.json({
    ...
    guideline_files: listGuidelineUploads(dir),
  });
}
...
export async function PATCH(req: NextRequest) {
  ...
  const { add_term, list = "do_not_change", wrong, correct, note } = body;
  const allowedLists = [
    "do_not_change",
    "brand_terms",
    "preferred_spellings",
  ...
  if (list === "style_guideline") {
    ...
    return NextResponse.json({ brand_kit: loadBrandKit(undefined, scope) });
  }
